...
description: >-
  Design behavior for default, development, release, and hotfix targets using supported auto-review controls. Use when this operator task needs a current, evidence-backed
  CodeRabbit workflow. Trigger with "configure CodeRabbit by branch".
...
tags: [saas, coderabbit, branches, environments, auto-review]
---
# CodeRabbit Branch and Environment Policy
...
- Repository YAML is read from the feature branch.
- The default target is always included when auto-review is enabled.
- `base_branches` adds target branches with regex patterns.
- Central, repo, organization, and global authorities have precedence.
...
1. Inventory branch patterns, risk, merge rules, and config authorities.

2. Define targets, drafts, labels, ignored authors, and path guidance.

3. Validate regexes and resolved feature-branch config on synthetic PRs.

4. Promote only after default, release, exclusion, and rollback cases pass.
